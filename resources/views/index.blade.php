...
<header>
	...
	@else
		<img src="{{(('images/preset/index.jpg'))}}" />
	@endif
	<div class="header">
		@if(!is_null($info->logo))
			<img src="{{(('images/'.$info->logo))}}" />
		@else
			<img src="{{(('images/preset/logo.jpg'))}}" />
		@endif
		<h1>{{$info->name}}</h1>
	</div>
</header>
		...
		<div class="row" style="margin: auto;" id="app">
			...
			@foreach($foods[$i] as $item)
				<div class="col-sm-12 col-md-6 col-lg-4 food-card">
					<div class="card">
						<img class="card-img-top" width="300" height="200" src="{{('images/'.$item->image)}}" alt="Card image cap">
...
{{--@if(!is_null(DB::table('orders')->where('token',Session::token())->orderBy('id','dsc')->first()))--}}
	<a v-show="showBill" class="float-left-btn" style="text-decoration: none" href="{{route('userBill',['id'=>Session::token()])}}">رسید سفارش</a>
{{--@endif--}}
</div>
@if(Auth::guard('cashier')->check())
	<a class="float-left-btn" style="text-decoration: none" href="{{route('orders')}}">پنل کاربری</a>
@elseif(Auth::guard('manager')->check())
	@if(\App\Restaurant::first()->complete == 1)
		<a class="float-left-btn" style="text-decoration: none" href="{{route('orders')}}">پنل کاربری</a>
	@else
		<a class="float-left-btn" style="text-decoration: none" href="{{route('settings')}}">پنل کاربری</a>
	@endif
@endif
<!-- The Modal -->
...
// $('.btnPlus').click(function(){
	...
	var hasTable = {!! json_encode($info->tableCounting) !!}
		...
		if(order.length > 0) {
			...
			var allPriceWithTax = parseInt(allPrice)+(parseInt(allPrice)*(parseInt(tax)/100));
			console.log("allPriceWithTax:" +allPriceWithTax)
			if(hasTable ==1) {
				htmlString = htmlString +`
         <div>

         	  <div id='tableDiv' class="flex-row space-between flex-center-align">
             ...
             <div class="flex-row space-between" style="margin-top: 1%;margin-bottom: 1%;">
               <span>مالیات بر ارزش افزوده :</span><span><b>`+tax+`%</b></span>
             </div>
             <div class="flex-row space-between" style="margin-top: 1%;margin-bottom: 1%;">
               <span>مجموع سفارش شما :</span><span><b>`+allPriceWithTax+` تومان</b></span>
             </div>
             <button onclick="checkOrder()" class="btn btn-warning" id="btnCheckOrder" disabled="true" style="display: block;margin: auto;">تایید سفارش</button>
             <button hidden=true onclick="sendOrder()" class="btn btn-success" id="btn" disabled="true" style="display: block;margin: auto;">ثبت سفارش</button>
             <button hidden=true type="button" class="btn btn-success" id="btn2" disabled="true" style="display: block;margin: auto;">ارسال سفارش</button>
           </div>
       `;
			} else {
				htmlString = htmlString +`
         <div>
             ...
             <button onclick="checkOrder()" class="btn btn-warning" id="btnCheckOrder" style="display: block;margin: auto;">تایید سفارش</button>
             <button hidden=true onclick="sendOrder()" class="btn btn-success" id="btn" style="display: block;margin: auto;">ثبت سفارش</button>
             <button hidden=true type="button" class="btn btn-success" id="btn2" disabled="true" style="display: block;margin: auto;">ارسال سفارش</button>
           </div>
       `;
			}
	...
	function tableId() {
		...
		if(document.getElementById('table_id').value == 0){
			...
			document.getElementById('btnCheckOrder').disabled=true;
		}else{
			document.getElementById('btnCheckOrder').disabled=false;
		}
	...
	function checkOrder() {
		document.getElementById('btnCheckOrder').hidden = true;
	...
	new Vue({
		...
		created:function () {
			...
			if({!! json_encode(DB::table('orders')->where('token',Session::token())->orderBy('id','dsc')->first()) !!} != null){
		this.showBill = 1;
	}
